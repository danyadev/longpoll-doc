...
* `mode` - Дополнительные опции ответа:
  ...
  * `512` - Возвращать бизнес-уведомления [TODO]
...
> флаг `1 << 10` (`1024`) присылается для обратной совместимости

Возможные флаги для сброса описаны в 12 событии
...
`1 << 8` (`256`) - Беседа с активным предложением на вступление (вы еще не вступили)

`1 << 9` (`512`) - Беседа с отклоненным предложением на вступление

`1 << 14` (`16384`) - Наличие упоминания (ответ на сообщение, пуш или `@all`) или исчезающего сообщения

`1 << 20` (`1048576`) - Наличие искусственной отметки беседы непрочитанной

`1 << 22` (`4194304`) - Беседа, в которую предложили вступить. Флаг остается, если:
  - приглашение не отозвано автором
  - приглашение вами отклонено или еще не принято
...
| 18  | Вы пригласили контакт или юзера в чат                          | `contactId` или `userId`    |

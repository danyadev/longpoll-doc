...
Последняя версия Long Poll - __11__, но в ней были удалены события [онлайна](#событие-8-друг-появился-в-сети) и [оффлайна](#событие-9-друг-вышел-из-сети) друга.
Вполне вероятно, что эта версия была создана для тестов.
...
Поле `history` идентично полю `updates`, которое приходит из Long Poll, за исключением сокращения некоторых событий:
...
т.е. структура уже из API, а не из LongPoll.
...
- [random_id](#зачем-нужен-random_id) - Зачем нужен `random_id`
...
[
  ...
  text: String,
  {
    ...
    has_template?: '1', // Наличие шаблона (для получения шаблона нужно получить сообщение из API)
    ...
    keyboard?: Object, // Клавиатура для ботов (в т.ч. инлайн)
...
Однако названия вложений, полученных через лонгпул, могут не совпадать с теми, что приходят через API:
- `event`, приходящий в API, в лонгпуле обозначается как `group`
...
- `artist`, `audio_playlist` и `article`, которые приходят в лонгпуле, через API отображаются как `link`

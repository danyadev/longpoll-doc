...
Судя по всему означает, что пользователь просмотрел свое упоминание в беседе.  
Насчет флагов я не уверен, но замечал только значение `17408` (`1024` и `16384`)

### Событие 12. Пришло упоминание в беседе

Структура: `[12, peer_id, flags]`

Приходит при упоминании или ответе на сообщение в беседе `peer_id`, однако не обещаю,
что не будет других кейсов вызова данного события с другими флагами.  
В поле `flags` я также замечал только `17408`.

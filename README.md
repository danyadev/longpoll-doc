...
При `actionType = 1`, 4-ый (с нуля) элемент массива содержит `id` реакции, которую поставил пользователь, а данные начинаются с 6-го элемента:
...
В остальных случаях данные начинаются с 5-го элемента:
...
#### Подробнее о массиве data

В целом, событие выглядит вот так:

```
[601,1,2000000153,1841767,2,5,5,4,2,2,443182555,63518289,4,32,1,1,138269465,6,2,3,3,131819250,172894294,647599618,4,15,1,1,355807901,4,36,1,1,49637977]
```

Для того, чтобы понять, как это событие обработать, давайте разделим массив на строки вот так:

```
[601,1,2000000153,1841767,2,5,
...
Итак, в первой строке находятся информация, которое было описано выше, а именно `type`, `actionType`, `peerId`, `conversationMessageId`, `myReactionId` (при `actionType = 1`) и `reactionsCount`. 

После первой строки идут ещё `reactionsCount` строк. Каждая строка описывает отдельную реакцию в сообщении. Возьмём четвёртую строку (т. е. 3-ю реакцию) из примера:
* 1-ый элемент — количество чисел в строке после него. В примере это `6`, т. е. после него нам надо парсить 6 элементов;
* 2-ой элемент — `id` реакции. В примере это `2`;
* 3-ий элемент — кол-во участников, которые поставили эту реакцию. В примере это `3`;
* 4-ый элемент — кол-во `id`-ов участников, которые идут далее. В примере это `3`, т. е. после этого элемента идут 3 элемента с `id`-ами участников, поставившие реакцию, а именно `131819250`, `172894294` и `647599618`.

__Обратите внимание!__ Если реакций в сообщении много, то будет приходить только кол-во участников без их `id`. В этом случае в 4-ом элементе будет значение 0, и он же будет последним в строке.

Выше был пример, где текущий пользователь поставил реакцию, и в сообщении содержалась `id` отреагировавших участников. Давайте теперь рассмотрим ещё один пример, когда другой участник поставил реакцию на сообщение, где реакций много и `id` отреагировавших нет:

```
[601,2,2000000153,1841767,10,
3,2,3,0,
3,4,2,0,
3,5,1,0,
3,6,1,0,
...
В этом примере значение `actionType` (второй элемент) равно `2`. В этом случае в первой строке отсутствует `myReactionId`, но в конце события появляется ещё одна строка: `0,6`. Она возвращается только при `actionType = 2`, и содержит `id` реакции и участника, поставившего её и вызвавший это событие. `id` реакции всегда находится в конце строки (и события). Т. к. `id` участников нет, то первый элемент равен `0`, а кол-во элементов в строке 2. А в ином случае первый элемент равно `1`, кол-во элементов в строке 3, а во 2-ом элементе будет `id` участника.

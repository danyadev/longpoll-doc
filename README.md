...
4. [Описание событий](#описание-событий)
   - [Описание сообщения](#описание-сообщения)
...
Так как многие события выдают именно сообщение, то стоит начать с его описания.
Данное описание не является валидным массивом, все сделано для наглядности.

- [Flags](#флаги-сообщений)
- [Action](#сервисные-сообщения)
- Keyboard
- Attachments
...
[
  ...
  text: String,
  {
    ...
    <Action> // Множество ключей для описания сервисного сообщения
  },
  {
    fwd?: '0_0', // При пересланных сообщениях и ответах на сообщения
    <Attachments> // Множестко ключей с описанием вложений
  },
  random_id: Number,
...
Сервисное сообщение описывается несколькими ключами в объекте. Основным является `source_act`, потому что он определяет само сервисное сообщение и все дополнительные ключи.

Возможные значения `source_act`:
- __`chat_photo_update`__ - Обновление фотографии беседы
   - Не имеет дополнительных ключей, но добавляет фотографию во вложениях
- __`chat_photo_remove`__ - Удаление фотографии беседы
   - Не имеет дополнительных ключей
- __`chat_create`__ - Создание беседы
   - `source_text` - Название беседы
- __`chat_title_update`__ - Обновление название беседы
   - `source_old_text` - Старое название беседы (но в сообщении из API его не будет)
   - `source_text` - Новое название беседы
- __`chat_pin_message`__ - Закрепление сообщения
   - `source_mid` - `id` закрепившего сообщения

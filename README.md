...
1. Пометка важным (`important` (`8`))
2. Пометка как спам (`spam` (`64`))
3. Удаление сообщения (`deleted` (`128`))
4. Удаление для всех (`deleted` (`128`) и `deleted_all` (`131072`))
5. Прослушивание голосового сообщения (`audio_listened` (`4096`))
...
Возможные значения [флагов сообщения](#флаги-сообщений):
1. Прочитано сообщение (`unread` (`1`)). Не рекомендую использовать, потому что иногда этот флаг не приходит
2. Отмена пометки важным (`important` (`8`))
3. Отмена пометки сообщения как спам (`spam` (`64`) и `cancel_spam` (`32768`))
4. Восстановление удаленного сообщения (`deleted` (`128`))

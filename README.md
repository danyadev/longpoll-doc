...
* `version` - Версия LongPoll, рекомендуется выставить 19.  
...
* `mode` - Настройка формата ответа. Рекомендуемое значение: `(1 << 1) | (1 << 3) | (1 << 5) | (1 << 7) | (1 << 9) | (1 << 10)`
...
| `1 << 5`  | Возвращать `pts`                                                                                      |
| `1 << 6`  | Возвращать события изменения онлайна друзей: [8] и [9] **[deprecated]**                               |
...
type LongPollResult =
  ...
  // updates - массив событий, которые описаны далее
...
Для получения истории событий нам необходим `pts`, который можно получить,
указав `need_pts: 1` при вызове метода [`messages.getLongPollServer`](https://dev.vk.com/method/messages.getLongPollServer)
и добавлением в `mode` флага `32` при [выполнении запроса](#подключение).

Для получения истории мы будем использовать метод [`messages.getLongPollHistory`](https://dev.vk.com/method/messages.getLongPollHistory) с указанием следующих параметров:
...
- `fields` - Поля [пользователей и групп](https://dev.vk.com/method/objects/user), которые могут прийти вместе с историей
...
Поле `history` идентично полю `updates`, которое приходит из LongPoll, за исключением удаления некоторых лишних событий
(т.к. приходит просто измененный объект беседы или сообщения) и сокращения некоторых событий:

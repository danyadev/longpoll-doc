...
5. [Описание событий](#описание-событий)
   ...
   - [Событие 504. Добавление беседы в папку](#событие-504-добавление-беседы-в-папку)
   - [Событие 505. Удаление беседы из папки](#событие-505-удаление-беседы-из-папки)
   - [Событие 506. Изменился порядок папок](#событие-506-изменился-порядок-папок)
...
__Это событие больше не приходит в LongPoll!__
...
Порядок такой: тип события, идентификатор папки, далее идут `peer_id` тех бесед, которые были добавлены в папку. Например — в папку с `id` 2 были добавлены две беседы с `peer_id` 2000000020 и 2000000192:

```
[504, 2, 2000000020, 2000000192]
```

### Событие 505. Удаление беседы из папки

Порядок такой же, как и в 504. Например — из папки с `id` 2 были удалены две беседы с `peer_id` 2000000020 и 2000000192:

```
[505, 2, 2000000020, 2000000192]
```
...
```
// folder_id, unread, unread_unmuted
[507, [1, 3, 2], [2, 1, 0]]
```

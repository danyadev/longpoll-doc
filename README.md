...
Однако данный список хакактерен только для сообщений из апи, у лонгпула же есть некоторые исключения:
- `event` обозначается как `group`
- `audio_message` и `graffiti` обозначается как `doc`, но добавляется ключ `attach*_kind` в ответе вложения со значением `audiomsg` или `graffiti`
...
function getAttachments(data) {
  ...
  for(const key in data) {
    ...
    if(match) {
      ...
      // Также здесь нужно обрабатывать link, но это сильно раздует код для примера.

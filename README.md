...
2. Сообщение исчезло - удаляется весь текст и вложения, добавляется ключ `is_expired: true`.
...
__ВАЖНО__: при исчезновении сообщения первым приходит именно 19 событие, поэтому перед проверкой на наличие ключа
`is_expired` в сообщении (а вы должны его как-то сохранить в сообщении) нужно подождать, пока обработается 18 событие.
Можно попробовать поставить небольшой таймер или воспользоваться своими средствами.
...
- __`chat_group_call_started`__ - Начало группового звонка в беседе
   - Нет дополнительных ключей
   - На данный момент приходит в LongPoll и отображается в сообщениях, полученных только от токена VK для Android
   - Приходит перед вызовом 115 события, которое означает начало группового звонка
...
__Хорошие новости:__ ВКонтакте, кажется, уже начал тестировать новый формат возвращения вложений - прямиком объект вложения как в API. На данный момент я заметил это только для стикеров и голосовых сообщений, но возможно в будущем все вложения будут приходить в подобном виде.
